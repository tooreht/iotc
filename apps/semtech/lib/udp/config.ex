defmodule UDP.Config do
  ...
  @doc """
  Same as get/3, but returns the result as a IP char list.
  If the value cannot be converted to a erlang IP tuple, the
  default is returned instead.
  """
  @spec get_ip(atom(), atom(), tuple()) :: tuple
  def get_ip(app, key, default \\ nil) do
    case get(app, key, nil) do
      nil -> default
      ip when is_tuple(ip) and tuple_size(ip) in [4,8] -> ip
      ip when is_bitstring(ip) ->
  ...
  If the environment variable is not set and no preconfigured default is provided
